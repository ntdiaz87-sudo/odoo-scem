...
class PartnerBalance(models.AbstractModel):
    ...
    def _lines(self, data):
        ...
        params = [self.env.context.get('lang'), tuple(data['ids']), tuple(data['computed']['result_selection'])] + tables_where_params[2]
        ...
        query = """
                    SELECT p.ref,account_move_line.account_id,coalesce(ac.name -> %s, ac.name -> 'en_US') AS account_name,ac.code AS code,p.name, sum(debit) AS debit, sum(credit) AS credit,
            ...
            WHERE (ac.id in %s) and (ac.account_type IN %s)       
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['form']['result_selection'] == 'customer':
            data['computed']['result_selection'] = ['asset_receivable']
        elif data['form']['result_selection'] == 'supplier':
            data['computed']['result_selection'] = ['liability_payable']
        else:
            data['computed']['result_selection'] = ['liability_payable', 'asset_receivable']
