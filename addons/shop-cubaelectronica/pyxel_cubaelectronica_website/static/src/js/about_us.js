...
publicWidget.registry.AboutUsMenu = publicWidget.Widget.extend({
    ...
    _onMenuItemClick(ev) {
        ...
        this.$el.closest('.about-cubaElectronica').find('.about-us-content > .section').removeClass('active');
        this.$el.closest('.about-cubaElectronica').find(`.about-us-content #${section}`).addClass('active');
    ...
    _activateFromHash() {
        ...
        const $li = this.$items.filter((_, li) => $(li).data('section') === sec);
